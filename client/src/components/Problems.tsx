        ...
        <div className="h-[120px] w-full cursor-pointer" onClick={openModal}>
          {problems.length === 0 ? (
            <div className="flex h-full w-full items-center justify-center">
              <h1 className="text-aod_text text-2xl font-bold">
                문제를 추가해주세요!
              </h1>
            </div>
          ) : (
            <div className="flex-col">
              {problems.map((problem, index) => (
                <div className="mt-1 flex h-[24px]" key={index}>
                  <div className="bg-aod_green/20 text-aod_green h-[24px] max-w-[368px] overflow-hidden overflow-ellipsis whitespace-nowrap rounded-[21px] px-2.5 py-1 text-left text-xs">
                    {problem.title}
                  </div>
                </div>
              ))}
